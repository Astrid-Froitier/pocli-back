...
const getAllRecipient = (async (
    ...
    try {
        ...
        const recipients = await Recipient.getAllRecipient(formatSortString(sortBy));
    ...
    const getOneRecipientById = (async (req: Request, res: Response, next: NextFunction) => {
        try {
      ...
      const getOneRecipientByName = (async (req: Request, res: Response, next: NextFunction) => {
        try {
          const { idRecipient } = req.params;
          const recipient = await Recipient.getRecipientByName(Number(idRecipient));
          recipient ? res.status(200).json(recipient) : res.sendStatus(404);
        } catch (err) {
          next(err);
        }
