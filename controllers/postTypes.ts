...
const getAllPostType = (async (
    ...
    const getOnePostTypeById = (async (req: Request, res: Response, next: NextFunction) => {
        try {
      ...
      const getOnePostTypeByName = (async (req: Request, res: Response, next: NextFunction) => {
        try {
          const { idPostType } = req.params;
          const postType = await PostType.getPostTypeByName(Number(idPostType));
          postType ? res.status(200).json(postType) : res.sendStatus(404);
        } catch (err) {
          next(err);
        }
